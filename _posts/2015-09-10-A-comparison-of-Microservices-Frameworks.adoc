...
All the underlying HTTP handling is done by the framework, you have no mandatory access to request and response objects. In this case, I create a Response but I could simply return the object; however, the return code would not be the right one (201), so to have full control over it, I prefer that solution. Moreover, the 404 (Status.NOT_FOUND) is set on the response; I could throw an exception instead, and write a mapper to make an adequate response, but it's overkill.

Note that injection is performed by Guice. It seems that there is a container provided with Jersey (hk2), but I didn't managed to make it work. Linking is handmade. 

Dropwizard uses Jackson to serialize / deserialize the object returned to JSON, so you have nothing special to do... but you have to configure the ObjectMapper to not fail on missing properties (see http://martinfowler.com/bliki/TolerantReader.html[Tolerant Reader]).

=== HTTPS

HTTPS is configured in the YAML configuration file. The documentation explains exactly how to set it up.
...
While a bit verbose due to all the plumbing involved in the setup, Dropwizard is a nice framework. It provides all the functions needed to build a MicroServices-based application. TO CONTINUE....


== Vertx

"`Vertx is a tool-kit for building reactive applications on the JVM.`". Built with Java 8, it is an asynchronous general purpose tool, build around reactive patterns, with which you can develop in Java of course, but also many languages running on the JVM (Javascript, Scala, Ruby, Python, Clojure).

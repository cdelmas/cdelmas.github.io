...
I already said that you have full control, did I? Well, as you can see, you indeed have full control (remember the raw servlet response?).
...
It consists of two classes, a `AuthHandler` and an `AuthProvider`. Once done, simply set the former as handler for all routes:
